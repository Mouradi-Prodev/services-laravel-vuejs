...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <meta name="keywords" content="services, social-services,social publishing, users, services, home services, {{$id}}, {{$title}}, {{$description}}">
        <meta property="og:title" content="{{$title}}">
        <meta property="og:description" content="{{$description}}">
        <meta property="og:type" content="website">
        <meta property="og:url" content="/service/{{$id}}/{{$title}}/{{$description}}">
                ...
                <li class="mr-3">
                    ...
                    <li class="mr-3">
                        <form id="logout-form" action="{{ route('logout') }}" method="POST" class="inline-block">
                            @csrf
                            <button type="submit" class="text-gray-600 no-underline hover:text-gray-200 hover:text-underline py-2 px-4">Logout</button>
                        </form>
                    </li>
                @endguest
            </ul>
